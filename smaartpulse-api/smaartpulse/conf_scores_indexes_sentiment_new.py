...
def sentiment_text_finder(x,negations_words_file):
    ...
    try:
        ...
        sentiment_text = sentence[0:]
    #print sentiment_text
    return sentiment_text

#quicker indexes function
def indexes_finder(x):
    row = list(x)
    review = row[16].replace("'"," ")
    sent_text = row[17]
    # finding the text start and end index in master string
    if sent_text in review:
        start_index = review.find(sent_text)
        end_index = start_index + len(sent_text)
    else:
        start_index = -1
        end_index = -1
    tuple_indexes = (start_index, end_index)
    #index_string = str(start_index) + delimiter + str(end_index)
...
def confidence_scores_indexes_sentiment(category,input_annotated_file,raw_reviews,negations_words_file,output_file,classifier):
    ...
    #quicker indexes function
    data_annotation_merge['new_indexes'] = data_annotation_merge.apply(lambda x: indexes_finder(x), axis = 1)
