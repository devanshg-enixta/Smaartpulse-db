...
def treemap_domain_polarity_changer(reviews_highlighted_file, domain_words_dir, source_reviews_outfile, treemap_outfile,
                                    category):
    ...
    df_empty = pd.DataFrame(
        columns=['source', 'source_review_id', 'source_product_id', 'product_name', 'review_date', 'star_rating',
                 'verified_user', 'reviewer_name', 'review_url', 'sentence', 'aspect', 'sentiment', 'aspect_keyword',
                 'sentiment_keyword', 'predictions', 'conf_score', 'review_text', 'sentiment_text', 'new_indexes',
    ...
    df_empty_3 = pd.DataFrame(
        columns=['source', 'source_review_id', 'source_product_id', 'product_name', 'review_date', 'star_rating',
                 'verified_user', 'reviewer_name', 'review_url', 'sentence', 'aspect', 'sentiment', 'aspect_keyword',
                 'sentiment_keyword', 'predictions', 'conf_score', 'review_text', 'sentiment_text', 'new_indexes',
                 'start_index', 'end_index', 'treemap_title'])
    ...
    for aspect_k, aspect_k_groups in d2.iteritems():
        ...
        dict_aspect_k_groups['aspect_' + aspect_k.strip()] = pd.DataFrame(
            list(aspect_k_groups.apply(sentiment_assigner, axis=1)),
            columns=['source', 'source_review_id', 'source_product_id', 'product_name', 'review_date', 'star_rating',
                     'verified_user', 'reviewer_name', 'review_url', 'sentence', 'aspect', 'sentiment',
                     'aspect_keyword', 'sentiment_keyword', 'predictions', 'conf_score', 'review_text',
                     'sentiment_text', 'new_indexes', 'start_index', 'end_index', 'treemap_title'])
    ...
    df_empty_3['treemap_name'] = df_empty_3['treemap_name'].apply(
        lambda x: x.replace("NEG_", "not ").replace("_", " ").strip())
    df_empty_3['source'] = 31
    df_empty_3['review_text_tag_ind'] = ""
    df_empty_3['category_id'] = 1
    ...
    cols_treemap = ["source", 'source_review_id', 'product_id', 'aspect', 'sentiment_text', 'review_text_tag_ind',
                    'start_index', 'end_index', 'confidence-score', 'source_product_id', 'treemap_name', 'category_id',
                    'sentiment_type']
    cols_source_review = ["source", 'source_review_id', 'source_product_id', 'product_name', 'review_date',
                          'star_rating', 'reviewer_id', 'reviewer_name', 'review_url', 'review_tag', 'review_text',
                          'product_id']
