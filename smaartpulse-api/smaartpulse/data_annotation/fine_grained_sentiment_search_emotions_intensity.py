...
def fine_grained_sentiment_search_emotions(phrase_dir, reviews_file,
                                  output_file, window,
    ...
    tester_file_4 = smart_open("/Users/enixtainnovationspvtltd/Documents/tags_aspects_test_4_454545.txt","wb")
    ...
    negation_words_list = load_negation_words(negation_words_file=
                                              negation_words_file)
    input_writer = unicodecsv.writer(infile, encoding='utf-8', delimiter='~')
    output_writer = unicodecsv.writer(outfile, encoding='utf-8', delimiter='~')

    # Iterating every line of the review file
    #print >> tester_file_3, phrase_dict
    print >> tester_file_4, global_dict
    print >> outfile, "source~source_review_id~source_product_id~product_name~review_date~star_rating~verified_user~reviewer_name~review_url~sentence~aspect~sentiment~aspect_word~sentiment_word"
    ...
    for line_num, line in enumerate(infile):
        ...
        #outfile_list.append(to_utf8(line.strip()))
        ...
        #NEW CODE FOR EMOTICONS!

        try:
            sentence = sentence.decode('utf-8')
            for x in emot.UNICODE_EMO:
                if x in sentence:
                    sentence = sentence.replace(x, ' ' + x + ' ')
        except Exception as Ex:
            pass

        toks1 = nltk.word_tokenize(sentence.decode('utf-8'))
        punctuations = ['!', '"', '#', '$', '%', '&', "'", '(', ')', '*', '+', ',', '-', '.', ':', ';', '<', '=', '>',
                        '?', '@', '[', ']', '^', '_', '`', '{', '|', '}', '~']
        toks1 = [x.encode('utf-8') for x in toks1 if x not in punctuations]
        # toks1 = list(tokenize(sentence, lowercase=True))
        temp_dict = dict()
        ...
        # toks = [x.encode('utf-8') for x in toks]
        #print toks
            ...
            for category in phrase_dict:
                ...
                for cat_type in phrase_dict[category]:
                    ...
                    if tag in phrase_dict[category][cat_type]:
                        ...
                        # VV temporary change for emotion intensity
                        if category == 'sentiments':
                            ...
                            print to_utf8(line.strip()), delimiter, aspect_tuple[1], delimiter, sent_tuple[1], delimiter, aspect_tuple[2], delimiter, sent_tuple[2]
                        ...
                        # For "aspects" and "sentiments" class of keywords
                            ...
                            if tag_new == 'NEG_' + tag_underscore:
                                ...
                                cat_type = change_emotion_type(cat_type)
            ...
            if aspect_tuple.category == 'aspects':
                ...
                if sent_tuple.category == "sentiments":
                    ...
                    if math.fabs(aspect_tuple[-1] - sent_tuple[-1]) < 15:
                        #outfile_list.append((aspect_tuple[0:-1], sent_tuple[0:-1]))
        ...
        #output_writer.writerow(outfile_list)
        print >> outfile, '\n',
        #print >> tester_file, "\n",
        #print >> tester_file, '\n'
...
if __name__ == "__main__":
    ...
    fine_grained_sentiment_search_emotions(phrase_dir=args.phrase_dir,
